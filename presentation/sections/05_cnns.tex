...
\begin{frame}{CNN Structure}
    % TODO: Add labeled CNN architecture diagram showing:
    % - Input image
    % - Convolutional layers (with filter sliding)
    % - Feature maps
    % - Pooling layers
    % - Fully connected layers at end
    % - Output
    % Label: kernels/filters, stride, feature maps, pooling windows
    
    \begin{center}
        \textit{[Diagram: CNN architecture with labeled components]}
    \end{center}
...
\begin{frame}{What Do Learned Kernels Look Like?}
    % TODO: Add visualization of learned kernels from first layer of CNN
    % Show grid of small filter visualizations (e.g., from AlexNet layer 1)
    % Should show edge detectors, color blobs, etc.
    
    \begin{center}
        \textit{[Visualization: learned kernels from CNN layer 1]}
    \end{center}
...
\begin{frame}{Kernels Applied to Images}
    % TODO: Add visualization showing:
    % - Original input image
    % - Several learned kernels
    % - The resulting feature maps (activations) when kernels are applied
    % Show that edge detector kernel produces edge-highlighted output, etc.
    
    \begin{center}
        \textit{[Visualization: kernel convolution on sample image → feature maps]}
    \end{center}
    
    \vspace{1em}
    \textbf{Each kernel produces a feature map:}
    \begin{itemize}
        \item Edge kernel → highlights edges in image
        \item Color kernel → highlights color regions
